...
VALIDATOR_REVISION = "phase4-v2-bundle-validator-v2"
...
def _with_receipt_identity(receipt: ValidationReceipt) -> ValidationReceipt:
    ...
    return replace(receipt, validation_receipt_sha256=hashlib.sha256(payload).hexdigest())
...
def _canonical_receipt_bytes(value: object) -> bytes:
    try:
        ...
        # Keep their receipts deterministic; accepted BOUND_V2 inputs are strict UTF-8.
        return json.dumps(
            value,
            sort_keys=True,
            separators=(",", ":"),
    ...
    try:
        ...
        return _with_receipt_identity(
            ValidationReceipt(
                ...
                validation_profile=(
                    "BOUND_V2" if expected_dependencies is not None else "FILESYSTEM_ONLY"
                ),
            )
        )
