...
SCHEMA_REVISION = "phase4-protocol-ir-core-v0.3.0-2026-08-31"
PROVENANCE_IDENTITY_REVISION = "phase4-provenance-identity-v2"
SUPPORTED_VALIDATOR_REVISION = "phase4-v2-bundle-validator-v2"
SUPPORTED_CONTRACT_REVISION = "phase4-v2-validation-input-v1"
BOUND_VALIDATION_PROFILE = "BOUND_V2"
...
class ValidatedReport:
    """Exact accepted BOUND_V2 receipt and all provenance attestations."""
...
def build_artifact_identity(
    *, package_id: str, version: str, artifact_digest: str
) -> ArtifactIdentity:
...
def bind_validator_receipt(
    ...
    *,
    ...
    """Bind a canonical BOUND_V2 receipt to explicit caller trust roots."""
...
def _parse_artifact_identity(raw: object, path: str) -> ArtifactIdentity:
    ...
    _expect_keys(
        value,
        path=path,
        required={"package_id", "version", "artifact_digest"},
    )
    package_id = _expect_string(value["package_id"], f"{path}.package_id")
    if not _PACKAGE_ID_PATTERN.fullmatch(package_id):
        _fail(
            "invalid_package_id",
            f"{path}.package_id",
            f"package ID {package_id!r} is not a dotted package identifier",
        )
    version = _expect_nonempty_string(value["version"], f"{path}.version", max_length=256)
...
def _parse_attested_anchors(raw: object, path: str) -> tuple[AttestedEvidenceAnchor, ...]:
    ...
    anchors = tuple(
        sorted(
            ...
            key=lambda anchor: _canonical_json(anchor.to_data()),
        )
    )
    ...
    for index, member in enumerate(report.validated_evidence_members):
        if member.owner != owner:
            _fail(
                ...
                "attested member owner does not match the preflight dependency pin",
            )
    for index, anchor in enumerate(report.validated_evidence_anchors):
        ...
        if anchor.owner != owner or anchor.owner != member.owner:
            _fail(
                ...
                "attested anchor owner does not match its member and preflight pin",
            )
...
def _validate_provenance(document: ProtocolIRDocument) -> None:
    ...
    for binding_id, binding in document.evidence_bindings:
        ...
        try:
            target_value = _resolve_json_pointer(semantic_data, binding.target)
            ...
            if isinstance(target_value, (dict, list)):
                diagnostics.append(
                    IRDiagnostic(
                        ...
                        f"{base_path}.target",
                        "evidence target must identify one scalar semantic leaf",
                    )
                )
