...
async def test_an_existing_bond_on_another_adapter_is_not_offered(
    ...
    assert options == ["pair_now"]
    assert "already has a Bluetooth bond" in result["description_placeholders"]["bond_state"]
...
async def test_replacement_holds_the_address_lock_until_pairing_finishes(
    ...
    with (
        ...
        result = await flow._async_replace_bond(
            flow._manual_data[CONF_ADDRESS], _bond_record()
        )
