...
_EVIDENCE_MEMBER = "work/decompiled/source.txt"
...
def _receipt_payload(**overrides: object) -> tuple[bytes, str]:
    receipt: dict[str, object] = {
        ...
        "validated_evidence_anchors": [
            {
                "id": "raise-summary",
                "owner": _DEPENDENCY_DIGESTS["preflight"],
...
def _provenance_document() -> tuple[dict[str, object], dict[str, str]]:
    ...
    data["evidence_anchors"] = {anchor_id: anchor.to_data()}
    data["source_sets"] = {source_set_id: source_set.to_data()}
    data["evidence_bindings"] = {binding_id: binding.to_data()}
    return data, {
        "package": package_id,
        "file": file_id,
        "anchor": anchor_id,
...
def test_validator_binding_retains_exact_receipt_attestations() -> None:
    ...
    assert report.validated_evidence_anchors[0].ir_pointer == "/actions/raise/summary"
...
def test_source_sets_cannot_cross_package_boundaries() -> None:
    ...
    report = _bind_report(bundle_sha256="2" * 64, report_manifest_sha256="3" * 64)
